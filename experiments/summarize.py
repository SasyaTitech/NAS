...
def _iter_checkpoint_records(run_dir: Path):
    ...
    for ckpt_dir in checkpoint_root.iterdir():
        if not ckpt_dir.is_dir() or not ckpt_dir.name.startswith("after_"):
            continue
        try:
            after = int(ckpt_dir.name.split("_", 1)[1])
        except ValueError:
            continue
        results_path = ckpt_dir / "results.jsonl"
        if results_path.exists():
            candidates.append((after, results_path))

    if not candidates:
        return None

    after, results_path = max(candidates, key=lambda x: x[0])
        ...
        with open(results_path, "r") as f:
            for line in f:
                ...
                try:
                    ...
                    print(
                        f"Could not decode a JSONL line in {results_path} due to format error; skipping."
                    )

    return after, results_path, _iter()


def _iter_case_json_records(run_dir: Path):
...
def summarize(
    ...
    #for run_dir in [Path(abs_path)]:
        ...
        checkpoint_data = _iter_checkpoint_records(run_dir)
        if checkpoint_data is not None:
            after, results_path, records_iter = checkpoint_data
            eval_source = f"checkpoint(after_{after})"
        else:
            files, records_iter = _iter_case_json_records(run_dir)
            eval_source = "root_case_json"

        for idx, data in enumerate(records_iter):
            if first_n_cases is not None and idx >= first_n_cases:
                break
