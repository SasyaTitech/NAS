...
def _maybe_memoir_prompt_boundaries(model, boundaries):
    """
    MEMOIR adapter inference requires a per-sample prompt boundary (0-based index of
    the last prompt token). For non-MEMOIR models this is a no-op.
    """
...
def test_batch_prediction(
    ...
    boundaries = torch.tensor(
        [l - 1 for l in prefix_lens for _ in range(2)],
...
def tfidf_similarity(text_a, text_b, vec):
    encs = vec.transform([text_a, text_b])
    if hasattr(encs, "toarray"):
        encs = encs.toarray()
    elif hasattr(encs, "todense"):
        encs = np.asarray(encs.todense())
    elif hasattr(encs, "A"):
        encs = encs.A
    norm = np.linalg.norm
    denom = norm(encs[0]) * norm(encs[1])
    if denom == 0:
        return 0.0
    return (np.dot(encs[0], encs[1]) / denom).item()
