...
@contextmanager
def _maybe_memoir_prompt_boundaries(model, boundaries):
...
def compute_rewrite_quality_zsre(
    ...
    rewrite_prompts = [fill_subject(record["requested_rewrite"]["prompt"], subject)]
    ...
    if getattr(model, "_memoir_adapter", None) is None:
        stuff_probs = test_batch_prediction_acc(model, tok, inp_prompts, inp_targets)
    else:
        base_prompt_lens = [len(x) for x in tok(inp_prompts_og)["input_ids"]]
        inp_prompt_boundaries = [
            l - 1 for l in base_prompt_lens for _ in range(len(target_tok))
        ]
        stuff_probs = test_batch_prediction_acc(
            model, tok, inp_prompts, inp_targets, prompt_boundaries=inp_prompt_boundaries
        )
...
def test_batch_prediction_acc(
    ...
    target,
    *,
    prompt_boundaries: typing.Optional[typing.List[int]] = None,
):
    prompt_tok = tok(
        prompts,
        padding=True,
    ...
    with torch.no_grad():
        if prompt_boundaries is None:
            boundaries = prompt_tok["attention_mask"].sum(1) - 1
        else:
            boundaries = torch.tensor(
                prompt_boundaries,
                device=prompt_tok["input_ids"].device,
                dtype=torch.long,
            )
