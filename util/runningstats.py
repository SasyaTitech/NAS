...
class Stat:
    ...
    def _normalize_add_shape_a(self, x, attr="data_shape_a"):
        """
        Flattens input data to 2d. Used for AB^T moments.
        """
        if not torch.is_tensor(x):
            x = torch.tensor(x)
        if len(x.shape) < 1:
    ...
    def _normalize_add_shape_b(self, x, attr="data_shape_b"):
        """
        Flattens input data to 2d. Used for AB^T moments.
        """
        if not torch.is_tensor(x):
...
class SecondMoment_AB_T(Stat):
    """
    Running computation for E[A B^T].
    """
    ...
    def add(self, a, b):
        a = self._normalize_add_shape_a(a)
        b = self._normalize_add_shape_b(b)
        if len(a) == 0 or len(b) == 0:
            return
        if self.count == 0:
            self.mom2 = a.new(a.shape[1], b.shape[1]).zero_()
        batch_count = a.shape[0]
        self.count += batch_count
        self.mom2 += a.t().mm(b)
