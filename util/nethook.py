...
class Trace(contextlib.AbstractContextManager):
    ...
    def __init__(
        ...
        def _infer_input_from_kwargs(kwargs):
            if not kwargs:
                return None
            for key in ("hidden_states", "x", "inputs", "input"):
                if key in kwargs:
                    return kwargs[key]
            tensor_items = [(k, v) for k, v in kwargs.items() if isinstance(v, torch.Tensor)]
            if not tensor_items:
                return None
        ...
        def _retain_hook(m, inputs, kwargs, output):
            if retain_input:
                if len(inputs) == 0 and kwargs is not None:
                    inferred = _infer_input_from_kwargs(kwargs)
                    input_to_retain = inferred if inferred is not None else inputs
                else:
                    input_to_retain = inputs[0] if len(inputs) == 1 else inputs
        ...
        def retain_hook(*hook_args):
            if len(hook_args) == 3:
                m, inputs, output = hook_args
                kwargs = None
            elif len(hook_args) == 4:
                m, inputs, kwargs, output = hook_args
            else:
                raise RuntimeError(f"Unexpected forward hook signature: {len(hook_args)} args")
            return _retain_hook(m, inputs, kwargs, output)
