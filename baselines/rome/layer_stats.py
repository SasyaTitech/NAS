...
_WIKIPEDIA_CONFIG_CANDIDATES = ("20231101.en", "20220301.en", "20200501.en")
...
def _load_hf_text_dataset(ds_name: str):
    ...
    if ds_name == "wikipedia":
        last_err = None
        for dataset_id in ("wikipedia", "wikimedia/wikipedia"):
        ...
        if last_err is not None:
            raise last_err
        raise ValueError(
            f"Unable to resolve wikipedia dataset config for ds_name={ds_name!r}"
        )
...
def main():
    ...
    aa(
        "--model_name",
        default="gpt2-xl",
        help="HuggingFace model id (e.g. 'gpt2-xl') or a local path.",
    )
    ...
    aa(
        "--device",
        default=None,
        help="Device to run on (e.g. 'cuda', 'cuda:0', or 'cpu'). Defaults to 'cuda' if available else 'cpu'.",
    )
