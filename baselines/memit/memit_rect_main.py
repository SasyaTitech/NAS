...
def apply_memit_rect_to_model(
    ...
    deltas, case_logs = execute_memit(
        model,
        tok,
        requests,
        hparams,
    ...
    with torch.no_grad():
        for w_name, (key_mat, val_mat) in deltas.items():
            ...
            if layer_logs is not None:
                updated_norm = torch.linalg.norm(w).item()
                layer_logs.append(
                    {
                        "layer": weight_to_layer.get(w_name),
                        "weight_name": w_name,
...
def execute_memit(
    ...
    cache_template: Optional[str] = None,
    edit_log_dir: Optional[str] = None,
) -> Tuple[Dict[str, Tuple[torch.Tensor]], Optional[List[Dict[str, Any]]]]:
    """
