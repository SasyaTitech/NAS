...
def apply_encore_to_model(
    ...
    deltas = execute_encore(model, tok, requests, hparams, cache_template=cache_template)
...
def execute_encore(
    ...
    """
    Executes the ENCORE update algorithm for the specified update at the specified layer.
    ...
    # Retrieve weights that r desires to change
    weights = {
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        if hparams.update_norm_lambda != 0:
            cov += hparams.update_norm_lambda * torch.eye(
                cov.shape[0], dtype=cov.dtype, device=cov.device
            )

        # Calculate effective covariance matrix (sequential editing)
...
def upd_matrix_match_shape(matrix: torch.Tensor, shape: torch.Size) -> torch.Tensor:
    ...
    if matrix.T.shape == shape:
        return matrix.T
    raise ValueError(
        "Update matrix computed by ENCORE does not match original weight shape."
    )
